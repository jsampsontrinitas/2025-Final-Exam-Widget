...
import { TestDataGroup } from "../../types/declaration";

export default class ExamTracker extends HTMLElement {
    modal: ModalOverlay;
    expanded: boolean;
    testData: TestDataGroup[];
    statusContainer: HTMLElement | undefined;
    ...
    initializeTestItemElements() {
        const testData = [] as TestDataGroup[];
    ...
    showTestDetailsInModal(testItem: TestItem) {
    ...
    addEventListeners() {
        ...
        this.shadowRoot!.addEventListener('click', (event) => {
            ...
            const item = target.closest<TestItem>('test-item');
            if (item) this.showTestDetailsInModal(item);
            if (target.closest('.status-header')) this.toggleExpand();
        });
